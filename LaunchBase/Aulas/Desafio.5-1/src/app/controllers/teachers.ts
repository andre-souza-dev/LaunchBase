...
import { getAge, getGraduation, getLessonType } from "../../lib/utils";

const teachersUrlLocation = "./src/data/teachers.json";
...
export default {
	async index(req: Request, res: Response) {
		fs.readFile(teachersUrlLocation, (error, data) => {
			if (error) {
	...
	async show(req: Request, res: Response) {
		...
		const teachers = JSON.parse(
			(await fs.readFileSync(teachersUrlLocation)).toString()
		);
	...
	post(req: Request, res: Response) {
		...
		fs.readFile(teachersUrlLocation, (error, data) => {
			...
			fs.writeFileSync(teachersUrlLocation, JSON.stringify(teachers, null, 4));
	...
	delete(req: Request, res: Response) {
		...
		fs.readFile(teachersUrlLocation, (error, data) => {
			...
			const teachers = JSON.parse(data.toString()) as Teacher[];
			const headers = req.headers["x-forwarded-for"];
			const ip =
				typeof headers === "object"
					? headers.shift()
					...
					? headers.split(",")[0]
					: "localhost";
			const foundTeachers = teachers.filter(
				(teacher) => teacher.id !== Number(id) || teacher.ip !== ip
			);

			fs.writeFileSync(
				teachersUrlLocation,
				JSON.stringify(foundTeachers, null, 4)
			);
		});
